...
namespace detail {
    ...
    constexpr uint32_t combine_crc32(uint64_t idx, const char * str, uint32_t part) {
        return (part >> 8) ^ crc_table[(part ^ str[idx]) & 0x000000FF];
    }

    constexpr uint32_t crc32(uint64_t idx, const char * str) {
