...
// negate operator
        ...
        const & q) {
        return
        fquatSIMD(_mm_mul_ps(q.Data, _mm_set_ps(-1.0f, -1.0f, -1.0f, -1.0f))
        );
    }
...
// operator+
    ...
    return
    fquatSIMD(_mm_add_ps(q1.Data, q2.Data)
    );
}
...
{
return
fquatSIMD(_mm_mul_ps(q.Data, _mm_set1_ps(s))
);
}

GLM_FUNC_QUALIFIER fquatSIMD

operator*(float s, fquatSIMD const &q) {
...
{
return
fquatSIMD(_mm_div_ps(q.Data, _mm_set1_ps(s))
);
}
...
template<typename T>
GLM_FUNC_QUALIFIER detail::fquatSIMD

quatSIMD_cast_impl(const T m0[], const T m1[], const T m2[]) {
...
{
return
quatSIMD_cast_impl(&m[0][0], &m[1][0], &m[2][0]
);
}
...
GLM_FUNC_QUALIFIER detail::fmat4x4SIMD
mat4SIMD_cast
(
        detail::fquatSIMD
const & q
...
return
_mm_mul_ps(_mm_add_ps(_mm_mul_ps(s0, x.Data), _mm_mul_ps(s1, y.Data)), d
);
}
}
...
{
return
detail::fquatSIMD(_mm_mul_ps(q.Data, _mm_set_ps(1.0f, -1.0f, -1.0f, -1.0f))
);
}
...
return
_mm_sub_ps(_mm_add_ps(_mm_sub_ps(x, y0), y1), y2
);
}
