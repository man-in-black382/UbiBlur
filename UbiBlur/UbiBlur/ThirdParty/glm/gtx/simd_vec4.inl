...
// operator+
        ...
        return
        fvec4SIMD(_mm_add_ps(v.Data, _mm_set1_ps(s))
        );
    }

    GLM_FUNC_QUALIFIER fvec4SIMD

    operator+(float s, fvec4SIMD const &v) {
    ...
    return
    fvec4SIMD(_mm_add_ps(v1.Data, v2.Data)
    );
}
...
{
return
fvec4SIMD(_mm_sub_ps(v.Data, _mm_set1_ps(s))
);
}
...
{
return
fvec4SIMD(_mm_sub_ps(v1.Data, v2.Data)
);
}
...
return
fvec4SIMD(_mm_mul_ps(par0, par1)
);
}
...
{
return
fvec4SIMD(_mm_mul_ps(v1.Data, v2.Data)
);
}
...
__m128 par1 = _mm_set1_ps(s);
return
fvec4SIMD(_mm_div_ps(par0, par1)
);
}
...
{
return
fvec4SIMD(_mm_div_ps(v1.Data, v2.Data)
);
}
...
{
return
fvec4SIMD(_mm_sub_ps(_mm_setzero_ps(), v.Data)
);
}
...
{
return
fvec4SIMD(_mm_add_ps(v.Data, glm::detail::one)
);
}
...
{
return
fvec4SIMD(_mm_sub_ps(v.Data, glm::detail::one)
);
}
...
//  return max(-a, a);
//}
GLM_FUNC_QUALIFIER detail::fvec4SIMD
abs
(
        detail::fvec4SIMD
const & x
...
detail::sse_clp_ps(x
.Data,
_mm_set1_ps(minVal), _mm_set1_ps(maxVal)
);
}
...
return
_mm_max_ps(_mm_min_ps(cmp0, _mm_setzero_ps()), detail::one
);
}
...
{
return
detail::sse_ssp_ps(edge0
.Data, edge1.Data, x.Data);
}
...
{
return
_mm_add_ps(_mm_mul_ps(a.Data, b.Data), c
.Data);
}
...
GLM_FUNC_QUALIFIER detail::fvec4SIMD
sqrt(detail::fvec4SIMD
const & x)
{
return
_mm_mul_ps(inversesqrt(x)
.Data, x.Data);
}

GLM_FUNC_QUALIFIER detail::fvec4SIMD
niceSqrt(detail::fvec4SIMD
const & x)
{
return
_mm_sqrt_ps(x
...
GLM_FUNC_QUALIFIER detail::fvec4SIMD
fastSqrt(detail::fvec4SIMD
const & x)
{
return
_mm_mul_ps(fastInversesqrt(x.Data)
.Data, x.Data);
}
...
GLM_FUNC_QUALIFIER detail::fvec4SIMD
inversesqrt(detail::fvec4SIMD
const & x)
{
GLM_ALIGN(4)
static const __m128 three = {3, 3, 3, 3}; // aligned consts for fast load
GLM_ALIGN(4)
static const __m128 half = {0.5, 0.5, 0.5, 0.5};
...
return
_mm_mul_ps(halfrecip, threeminus_xrr
);
}

GLM_FUNC_QUALIFIER detail::fvec4SIMD
fastInversesqrt(detail::fvec4SIMD
const & x)
{
return
_mm_rsqrt_ps(x
